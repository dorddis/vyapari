...
Prompt template for the gap/completeness review agent launched by `$commit`.

Inject:
- The actual diff into `{DIFF}`
- The changed file list into `{FILES_CHANGED}`
- Any available task context (from STATUS.md, memory, or conversation) into `{TASK_CONTEXT}`
