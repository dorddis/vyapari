...
- **GPT-5.4-mini:** Lightweight classification (escalation detection fallback when phrase matching is ambiguous, command-vs-relay safety check)
- **GPT-4o Transcribe:** Voice note STT (stretch goal)
...
| Phrase matching over ML for escalation | Simple, fast, no training data needed | Nuanced detection (sarcasm, implied frustration) | 15-20 trigger phrases cover 90%. GPT-5.4-mini as fallback. |
