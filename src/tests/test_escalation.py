...
async def test_test_drive_triggers():
    triggered, reason = await detect_escalation("Kal test drive le sakta hu?", "Sure!")
...
async def test_best_price_triggers():
    triggered, reason = await detect_escalation("Best price kya hai?", "Listed at 7.5L.")
    assert triggered is True
...
async def test_baat_karo_triggers():
    triggered, reason = await detect_escalation("Kisi se baat karo", "Of course!")
    assert triggered is True
...
async def test_bot_offering_connect_triggers():
    triggered, reason = await detect_escalation(
        "I need more details",
